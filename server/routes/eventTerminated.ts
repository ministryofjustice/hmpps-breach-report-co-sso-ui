...
export default function eventTerminatedRoutes(
  ...
  router.post('/event-terminated/:id', async (req, res) => {
    ...
    if (req.body.action === 'delete') {
      ...
      try {
        const cosso = await cossoApiClient.getCossoById(id as string, res.locals.user.username)
        ...
        await cossoApiClient.deleteCosso(id as string, res.locals.user.username)
      } catch (error) {
